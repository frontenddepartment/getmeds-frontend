...
export default function ShippingAndDeliveryPolicy() {
  ...
  useEffect(() => {
    document.title = 'Shipping & Delivery Policy — Getmeds';
    ...
    canonicalEl.setAttribute('href', 'https://getmeds.ph/shipping-and-delivery-policy');
    ...
    getPolicyBySlug('shipping-and-delivery-policy')
      .then((data) => {
        if (data?.contentHtml) {
      ...
      .catch((err) => {
        console.warn('Failed to fetch Shipping & Delivery Policy from Sanity database:', err);
      });
  }, []);
        ...
        <article className="min-w-0">
          {dynamicHtml ? (
            ...
            <div className="p-6 bg-white rounded-xl shadow-sm border border-gray-100">
              <h2 className="text-xl font-bold mb-3 text-gray-900">Shipping &amp; Delivery Policy</h2>
              <p className="text-gray-700 text-sm leading-relaxed mb-4">
                Getmeds provides nationwide pharmaceutical logistics and delivery services under Good Distribution Practice (GDP) standards.
              </p>
            </div>
          )}
        </article>
